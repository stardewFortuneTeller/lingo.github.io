...
const returner = [["LEVEL 1", 26, "white", 1]];
...
const l1puzzles = startingRoom.concat(
  heightTutorial,
  theTenacious,
  theArtisticPanda,
  theBold,
...
const l2puzzles = returner.concat(
  startingTwoom,
  theFuzzy,
  theExemplary,
  theFresh,
...
const l10001puzzles = returner.concat(theRedundant, theNaive);

let puzzles = l1puzzles;
let levelNo = 1;
...
let randomElement = 1;
let shufflePuzzles = 0;
let completeInOrder = 1;
...
function validate() {
  let x = document.getElementById("answer-field").value;
  ...
  if (x == puzzles[randomElement][1]) {
    ...
    audio.play();
    let puzzleID = puzzles.indexOf(puzzles[randomElement]);
...
function setupGame() {
  ...
  if (completeInOrder == 0) {
    ...
    if (oldRand == randomElement) {
      randomElement++;
      if (randomElement == puzzles.length) {
        randomElement = 0;
      }
    }
  } else {
  ...
  if (completedPuzzles.includes(randomElement)) {
    document.getElementById("clue").style.color = "rgb(0, 255, 0)";
  }
  ...
  if (completedPuzzles.includes(randomElement)) {
    document.getElementById("answer-field").value = puzzles[randomElement][1];
  }
  help = 0;
  let placeholderTxt = "";
...
function moveRight() {
  if (randomElement != puzzles.length - 1) {
    setupGame();
  }
}
...
function loadCompletedPuzzles() {
  const completedPuzzlesJSON = localStorage.getItem(
    "completedPuzzles" + levelNo
  );
